"""`lens_quiet_living_hamburg_insight` — one-call lens-level executive
summary for the Quiet Living lens of the Hamburg address-intelligence
tool.

Hamburg variant — duplicate-edit per spec Q5. Berlin QL template
(`lens_quiet_living_insight.py`) untouched.

Mirrors the Berlin QL summariser architecture (schema, deterministic
rollup, highlight-tone filter, fit_score, two-attempt retry) with these
Hamburg-specific adaptations:
- Audience framing: people prioritising a calm, low-noise home in
  Hamburg (Bezirk + Stadtteil naming, not Ortsteil).
- 8 tiles grouped into 4 sections. Removed vs Berlin QL: `noise`,
  `air`, `rail_noise`, `gesix_quiet`. Added: `heat` (Stadtklimaanalyse
  2023 PET day-class) and `sozialmonitoring_status_quiet` (BSW
  Sozialmonitoring Statusindex — real tiered shape-only tile).
- Inverted-signal handling: `arterial_road`, `cobblestone_nearby`,
  `nightlife_inverted` read "further / fewer is better". `rail_noise`
  is intentionally absent — the Berlin heuristic treats U-Bahn as
  always green (mostly underground); HH's U-Bahn has long elevated
  stretches so the shortcut inverts on HH addresses.
- `sozialmonitoring_status_quiet` is a real tiered signal — treat as a
  citable positive / concern the same way transit tiles are, not as
  info-only.

DOES NOT dynamically import sibling `*_insight.py` templates. Consumes
only the tile payload fields the app proxy sends: `label`, `tier`,
`rule`, `numeric`, `caveat`.

Response schema (identical to Berlin QL):

    {
      ...
      "highlights_green": [{"tile": "quiet_zone",
                            "one_line": "320 m to Öjendorfer Park."}, ...],
...
SAMPLER = {
    ...
    # 1400 tokens covers an 8-tile lens payload with 4 sections + up to
    # 6 highlights without truncation on Cloudflare Llama-3.1-8B-fast.
    "max_tokens":         1400,
}

# Hamburg Quiet Living tile groupings. Every key must match a tile
# emitted by `quiet_living_lens` in app/core/lenses/quiet_living.py for
# the Hamburg city config (`QUIET_LIVING_LENS` in
# `app/cities/hamburg/lenses.py`). The __main__ selfcheck's
# completeness assert enforces coverage.
LENS_SECTION_MAP: list[dict] = [
    {"title": "Green refuge",
     "tiles": ["quiet_zone", "street_trees"]},
    {"title": "Traffic pressure",
     "tiles": ["tempo30", "arterial_road", "cobblestone_nearby"]},
    {"title": "Climate & nightlife",
     "tiles": ["heat", "nightlife_inverted"]},
    {"title": "Neighbourhood profile",
     "tiles": ["sozialmonitoring_status_quiet"]},
]
...
_SYSTEM = (
    ...
    "lens of a Hamburg address-intelligence tool. The user is someone "
    ...
    "(a) the address's Bezirk / Stadtteil hint, (b) a list of 8 Quiet "
    ...
    "`sozialmonitoring_status_quiet` is a real neighbourhood-status "
    "tier (Hamburg BSW Sozialmonitoring — 4-level Statusindex per "
    "Statistisches Gebiet ~2200 residents) and IS eligible to be cited "
    "as a positive or concern the same way traffic / refuge tiles are. "
    "`heat` reads the Hamburg Stadtklimaanalyse 2023 PET day-class for "
    "the residential block; a red heat reading is a real quiet-living "
    "concern (hot addresses mean windows shut in summer, so outdoor "
    "noise leaks in through opened windows on cooler nights). "
    "IMPORTANT — section names vs tile names: `expected_sections[].title` "
    "values ('Green refuge', 'Traffic pressure', 'Climate & nightlife', "
    ...
    "using the tile `label` (e.g., 'nearest quiet zone', "
    "'street-tree density', 'arterial road', 'summer heat'), NOT the "
    "section title that contains it. Writing 'The Traffic pressure is "
    "high' is WRONG — that conflates a section grouping with specific "
    "tiles. Write 'The nearest arterial road is 40 m away' instead. "
    ...
    "`cobblestone_nearby`, and `nightlife_inverted` are inverted "
    "(further / fewer is better). If the tile's `caveat` mentions "
    "'INVERTED' or 'further is better' or 'fewer is better', do NOT "
    "describe a green reading as 'far / lacking' — describe it as "
    "'buffered from', 'well set back from', or 'few venues nearby'. "
    ...
    "No German words except proper names (Bezirk, Stadtteil, "
    "Ruhige Gebiete, Ruheinseln, Straßenbaumkataster, Sozialmonitoring, "
    "Statistisches Gebiet, Statusindex, Stadtklimaanalyse are fine as "
    "terminology). "
...
def build_messages(context: dict) -> list[dict]:
    ...
    Expected `context` shape:
      {
        "address_hint": {"bezirk": "...", "stadtteil": "..."},
        "tile_contexts": [
          {"key": "quiet_zone", "label": "Nearest quiet zone",
           "tier": "green", "rule": "≤400 m green",
           "numeric": "320 m to Öjendorfer Park", "caveat": None},
          ...
        ],
      }
    """
    ...
    # Compact placeholder exemplar_user — full 8-tile listing dropped to
    # keep the prompt lean. Genericised bezirk / stadtteil so smaller
    ...
    exemplar_user = json.dumps({
        "address_hint": {"bezirk": "<Bezirk from payload>",
                          "stadtteil": "<Stadtteil from payload>"},
        "expected_sections": LENS_SECTION_MAP,
        "tiles": "<8 Quiet Living tiles with key/tier/rule/numeric/caveat>",
    }, ensure_ascii=False)

    exemplar_assistant = json.dumps({
        "executive_summary": (
            "This Stadtteil reads mixed on the Quiet Living axes: the "
            "nearest arterial road is 80 m away, so the block carries "
            "some daytime traffic pressure, and the summer heat class "
            "sits at mäßige Belastung. On the calmer side, a designated "
            "Ruhige Gebiete zone is a 10-minute walk, street-tree "
            "density is high, and only a couple of late-night venues "
            "fall within earshot — so the daily buffer is close even "
            "if the front door faces a busier street."
        ),
        "sections": [
            {"title": "Green refuge",
             "tiles": ["quiet_zone", "street_trees"],
             "verdict": "green",
             "note": "Designated quiet zone reachable on foot; street-tree density is high on the block."},
            {"title": "Traffic pressure",
             "tiles": ["tempo30", "arterial_road", "cobblestone_nearby"],
             "verdict": "amber",
             "note": "30 km/h street; arterial buffered but present; no cobblestone streets within earshot."},
            {"title": "Climate & nightlife",
             "tiles": ["heat", "nightlife_inverted"],
             "verdict": "amber",
             "note": "Summer heat class is mäßige Belastung; few late-night venues within 300 m."},
            {"title": "Neighbourhood profile",
             "tiles": ["sozialmonitoring_status_quiet"],
             "verdict": "green",
             "note": "Sozialmonitoring Statusindex is hoch for this Statistisches Gebiet."},
        ],
        "highlights_green": [
            {"tile": "quiet_zone",
             "one_line": "Ruhige Gebiete zone 320 m from the door."},
            {"tile": "street_trees",
             "one_line": "82 registered street trees within a 200 m disk — tree-dense."},
        ...
        "highlights_red": [
            ...
            {"tile": "heat",
             "one_line": "Summer heat class is mäßige Belastung on this residential block."},
        ],
    }, ensure_ascii=False)
...
def run(backend, context: dict) -> dict:
    ...
    raise ValueError(
        f"lens_quiet_living_hamburg_insight: model returned invalid JSON twice "
        f"({type(last_err).__name__}: {last_err})"
    )


if __name__ == "__main__":
    # -- LENS_SECTION_MAP covers all 8 Hamburg Quiet Living tile keys,
    # no orphans, no duplicates. Guards against a new HH QL tile landing
    # in `app/cities/hamburg/lenses.py::QUIET_LIVING_LENS` without a
    # matching section entry here.
    _ql_tile_keys = {
        "quiet_zone", "street_trees",
        "tempo30", "arterial_road", "cobblestone_nearby",
        "heat", "nightlife_inverted",
    ...
    assert set(_mapped) == _ql_tile_keys, \
        f"LENS_SECTION_MAP diverges from Hamburg Quiet Living tile keys — " \
    ...
    _ctx = {
        "address_hint": {"bezirk": "Eimsbüttel", "stadtteil": "Rotherbaum"},
        "tile_contexts": [
            {"key": "quiet_zone", "label": "Nearest quiet zone",
             "tier": "green", "rule": "≤400 m green",
             "numeric": "320 m to Öjendorfer Park"},
    ...
    assert "inverted" in _sys.lower(), \
        "system must handle INVERTED signals (arterial_road, cobblestone_nearby, nightlife_inverted)"
    assert "buffered" in _sys.lower(), \
        "system must model correct inverted-signal framing (e.g. 'buffered from')"
    # Hamburg-specific: Stadtteil (not Ortsteil) + sozialmonitoring
    # framing must be explicit so the model knows the QL-quiet variant
    # of the tile is a real tier, not info-only.
    assert "Stadtteil" in _sys, \
        "system must use Stadtteil (Hamburg) terminology, not Ortsteil"
    assert "sozialmonitoring_status_quiet" in _sys, \
        "system must call out sozialmonitoring_status_quiet as a real tier"
    assert "heat" in _sys.lower() and "stadtklimaanalyse" in _sys.lower(), \
        "system must frame the heat tile with its Stadtklimaanalyse source"
    ...
    assert "Eimsbüttel" in _real_user
    assert "Rotherbaum" in _real_user
    ...
    # Stadtteil (not ortsteil) in address_hint.
    assert "stadtteil" in _real_user, \
        "real user payload must use stadtteil key, not ortsteil"
    ...
    _input_ctx = {
        ...
        "tile_contexts": [
            ...
            {"key": "cobblestone_nearby", "tier": "green"},
            {"key": "heat", "tier": "amber"},
            {"key": "nightlife_inverted", "tier": "green"},
            {"key": "sozialmonitoring_status_quiet", "tier": "green"},
        ],
    }
    _out = run(_b, _input_ctx)
    assert _b.calls == 1, "valid response should not retry"
    # Section 0 (Green refuge = quiet_zone green + street_trees green) →
    # green (model said red, rollup corrected).
    assert _out["lens_insight"]["sections"][0]["verdict"] == "green", \
        f"rollup should have overridden model 'red' to 'green', got " \
        f"{_out['lens_insight']['sections'][0]['verdict']!r}"
    # Section 1 (Traffic pressure = tempo30 green + arterial amber +
    # cobblestone green) rolls up to amber.
    assert _out["lens_insight"]["sections"][1]["verdict"] == "amber", \
        "Traffic pressure rolls up to amber from arterial_road amber"
    # Section 2 (Climate & nightlife = heat amber + nightlife green)
    # rolls up to amber.
    assert _out["lens_insight"]["sections"][2]["verdict"] == "amber", \
        "Climate & nightlife rolls up to amber from heat amber"
    # Section 3 (Neighbourhood profile = sozialmonitoring green) rolls
    # up to green.
    assert _out["lens_insight"]["sections"][3]["verdict"] == "green", \
        "Neighbourhood profile rolls up to green from sozialmonitoring_status_quiet"
    # Fit score = (green, amber, amber, green) → (100+60+60+100)/4 = 80.
    ...
    print("lens_quiet_living_hamburg_insight.py selfcheck OK")
