...
_SYSTEM = (
    ...
    "permit regime) and `xmas_market` (seasonal signal). When either "
    "appears in a section's tile list, the note MUST cite it. Adapt "
    "to the tile's rule; example phrasings (paraphrase to fit the "
    "sentence, do not copy verbatim, do not invent a month or a "
    "count not present in the payload): "
    "parkzone green + rule contains 'permit priority for residents' → "
    "'inside a Parkzone (residents get a Bewohnerparkausweis for "
    "~€10/yr)'; parkzone green + rule contains 'free street parking' "
    "→ 'street parking is unregulated here — no Bewohnerparkausweis "
    "needed'; parkzone amber → 'close to a paid zone edge'. "
    "xmas_market green (N Christmas markets within 3 km, N from the "
    "tile's rule/numeric) → 'N Christmas markets within 3 km'; "
    "xmas_market amber (exactly 1 within 3 km) → '1 Christmas market "
    "within 3 km'; xmas_market red → 'no Christmas markets within "
    "3 km'."
)
