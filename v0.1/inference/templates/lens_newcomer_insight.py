...
_SYSTEM = (
    ...
    "`caveat`, and `framing` (an excerpt from the per-tile prompt that "
    "encodes how to interpret this signal). "
...
def _load_tile_framings() -> dict:
    """Load a short excerpt from each Newcomer tile template's SYSTEM
    prompt at import time. This is the "compile-time template reuse" the
    architecture spec calls for — sibling per-tile templates stay live
    (they still serve the /summarize endpoint for their own paths), and
    their interpretive framing is inherited by this lens summariser
    without a runtime LLM fan-out.

    Missing sibling (e.g., `nightlife_density` has no template) → empty
    string; the summariser then relies on `rule` + `numeric` + `caveat`.
    Excerpt length is 220 chars — enough to carry the framing intent
    without inflating the fan-in prompt beyond CF context budget."""
    _EXCERPT = 90
    framings: dict = {}
    _mapping = [
        ("buergeramt",       "buergeramt_insight"),
        ("rail_transit",     "rail_transit_insight"),
        ("tram_transit",     "tram_transit_insight"),
        ("bus_transit",      "bus_transit_insight"),
        ("intl_food",        "intl_food_insight"),
        ("coworking",        "coworking_insight"),
        ("english_clinic",   "english_clinic_insight"),
        ("language_school",  "language_school_insight"),
        ("library",          "library_insight"),
        ("packstation",      "packstation_insight"),
        ("wochenmarkt",      "wochenmarkt_insight"),
        ("gesix_newcomer",   "gesix_newcomer_insight"),
        # nightlife_density has no sibling template — numeric-only tile.
        ("nightlife_density", None),
    ]
    for tile_key, template_module in _mapping:
        if template_module is None:
            framings[tile_key] = ""
            continue
        try:
            mod = __import__(f"inference.templates.{template_module}",
                             fromlist=["_SYSTEM"])
            sys_prompt = getattr(mod, "_SYSTEM", "") or ""
            framings[tile_key] = sys_prompt[:_EXCERPT]
        except Exception:
            # If a sibling template can't be imported, don't take down
            # the lens template — just carry an empty framing for that
            # tile. Prevents a broken sibling from cascading.
            framings[tile_key] = ""
    return framings


# Loaded once at import — sibling `_SYSTEM` prompts are module-level
# constants that never change per-request, so no reason to re-read them
# per call.
_TILE_FRAMINGS = _load_tile_framings()
...
def _build_tile_payload(tile_contexts: list[dict]) -> list[dict]:
    ...
    verdict axis, not the tile drill-down. Enriches each tile with its
    framing excerpt from `_TILE_FRAMINGS` so section notes inherit the
    same interpretive language as the per-tile templates."""
    out = []
    for tc in tile_contexts or []:
        ...
        out.append({
            ...
            "framing":  _TILE_FRAMINGS.get(key, ""),
        })
    return out
...
def build_messages(context: dict) -> list[dict]:
    ...
    Expected `context` shape:
      {
        ...
        "tile_contexts": [
          {"key": "buergeramt", "label": "Bürgeramt reach",
           ...
           "caveat": None, "framing": "<first 200 chars of tile SYSTEM>"},
          ...
        ],
      }
    """
    ...
    exemplar_user = json.dumps({
        ...
        "expected_sections": LENS_SECTION_MAP,
        "tiles": "<13 Newcomer tiles with key/tier/rule/numeric/caveat/framing>",
    }, ensure_ascii=False)
...
if __name__ == "__main__":
    # -- _TILE_FRAMINGS loaded at import; every mapped tile has an entry
    # (even if empty), and the excerpt is non-empty for every tile that
    # has a sibling template. If a sibling template disappears or renames
    # its `_SYSTEM` constant, this assert catches it.
    assert set(_TILE_FRAMINGS.keys()) >= {
        "buergeramt", "rail_transit", "tram_transit", "bus_transit",
        "intl_food", "coworking", "english_clinic", "language_school",
        "library", "packstation", "wochenmarkt", "gesix_newcomer",
        "nightlife_density",
    }, "_TILE_FRAMINGS missing a Newcomer tile key"
    # Every tile except nightlife_density (numeric-only, no template)
    # should have a non-empty framing excerpt.
    for _k, _v in _TILE_FRAMINGS.items():
        if _k == "nightlife_density":
            continue
        assert _v, f"_TILE_FRAMINGS[{_k!r}] is empty — sibling template load failed"
        assert len(_v) <= 90, f"framing excerpt for {_k} exceeds 90 chars"
