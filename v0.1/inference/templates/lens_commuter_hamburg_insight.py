"""`lens_commuter_hamburg_insight` — one-call lens-level executive summary
for the Commuter lens of the Hamburg address-intelligence tool.

Hamburg variant — duplicate-edit per spec Q5. Berlin file untouched.

Consumes ALL 11 Commuter tile scoring outputs at once and returns a strict-
...
Response schema:

    {
      ...
      "highlights_green": [{"tile": "commuter_rail_transit",
                            "one_line": "S-Bahn 180 m from the door"}, ...],
      "highlights_red":   [{"tile": "airport_reach",
                            "one_line": "HAM is 18 km away"}, ...],
...
- No info-only tiles in Hamburg Commuter — all 11 tiles emit real
  green / amber / red / unknown. `info` is still accepted in `_TIER_RANK`
  as an alias for `unknown` for parity with `lens_newcomer_hamburg_insight`.
  `sozialmonitoring_status_commuter` and `sozialmonitoring_gesamt_commuter`
  carry real tiers (Hamburg BSW Sozialmonitoring — 4-level Statusindex per
  Statistisches Gebiet ~2200 residents); they only land as unknown when the
  address falls outside a published polygon.

Cache history
-------------
v1: initial Hamburg Commuter template
"""
...
# flowing prose. max_tokens fits 5 sections + up to 6 highlights on an
# 11-tile lens without truncation.
SAMPLER = {
    "temp":               0.35,
    "top_p":              0.9,
...
LENS_SECTION_MAP: list[dict] = [
    ...
    {"title": "Short-hop transit",
     "tiles": ["commuter_ferry_transit", "commuter_bus_transit"]},
    {"title": "Cycling & car",
    ...
    {"title": "Neighbourhood profile",
     "tiles": ["sozialmonitoring_status_commuter", "sozialmonitoring_gesamt_commuter"]},
]
...
# `lens_newcomer_hamburg_insight` — Hamburg Commuter has no info-only
# tiles today (all 11 tiles emit real tiers, including `airport_reach`
# which uses km-based thresholds and `sozialmonitoring_*` which use
# Hamburg BSW Sozialmonitoring 4-level Statusindex). The `.get(tier, -1)`
# default excludes any non-ranked value from the rollup so a section of
# only unknown tiles rolls up to `unknown`.
...
_SYSTEM = (
    ...
    "of a Hamburg address-intelligence tool. The user commutes daily and "
    "cares about door-to-transit time, cycling network reach, and "
    "multi-modal options (S+U-Bahn, ferry, bus, regional rail, cycling, "
    ...
    "address's Bezirk / Stadtteil hint, (b) a list of 11 Commuter tile "
    ...
    "drawn from tiered tiles (green / amber / red). All 11 Hamburg "
    "Commuter tiles carry real tiers — every tile is eligible to be "
    "cited as a positive or concern. "
    "`sozialmonitoring_status_commuter` and "
    "`sozialmonitoring_gesamt_commuter` are real neighbourhood-status "
    "tiers (Hamburg BSW Sozialmonitoring — 4-level Statusindex per "
    "Statistisches Gebiet ~2200 residents; Aufmerksamkeitsgebiet is a "
    "combined Status+Dynamik verdict) and ARE eligible to be cited as "
    "positives or concerns the same way transit tiles are. "
    ...
    "reach', 'ferry reach', 'cycling network', 'car-sharing'), NOT the "
    "section title that contains it. Writing 'The Rail & regional is "
    "strong' is WRONG — that conflates a section grouping with a "
    "specific tile. Write 'S+U-Bahn is 180 m away' instead. "
    "No German words except proper names (Bezirk, Stadtteil, S-Bahn, "
    "U-Bahn, HVV, HADAG, Elbe, Sozialmonitoring, Aufmerksamkeitsgebiet "
    "are fine as terminology). "
    ...
    "value is high (Hamburg resident permit regime) and it easily gets "
    ...
    "verbatim, do not invent a fee or enforcement hours not present in "
    "the payload — the Hamburg WFS does not publish those): "
    "parkzone green + rule contains 'permit priority for residents' → "
    "'inside a Bewohnerparkgebiet (residents get a Bewohnerparkausweis)'; "
    "parkzone green + rule contains 'no zone' → "
    "'street parking is unregulated here — no Bewohnerparkgebiet applies'; "
    "parkzone amber → 'close to a Bewohnerparkgebiet edge'."
)
...
def build_messages(context: dict) -> list[dict]:
    ...
    # bezirk / stadtteil in the exemplar was being copied into the
    ...
    exemplar_user = json.dumps({
        ...
        "expected_sections": LENS_SECTION_MAP,
        "tiles": "<11 Commuter tiles with key/tier/rule/numeric/caveat>",
    }, ensure_ascii=False)

    exemplar_assistant = json.dumps({
        "executive_summary": (
            "This Stadtteil reads strong on the Commuter axes that matter "
            "most day to day: S-Bahn is a 3-minute walk, an HADAG ferry "
            "pier is on your doorstep, and cycling infrastructure is "
            "dense. The main gap is airport reach — HAM is 18 km away, "
            "so a monthly airport trip needs the S1 or a longer taxi "
            "ride. Regional rail is reachable 1.4 km if long-haul travel "
            "matters."
        ),
        "sections": [
            {"title": "Rail & regional",
             "tiles": ["commuter_rail_transit", "regional_rail_reach"],
             "verdict": "amber",
             "note": "S+U-Bahn on your doorstep; regional rail is 1.4 km — a short bike ride."},
            {"title": "Short-hop transit",
             "tiles": ["commuter_ferry_transit", "commuter_bus_transit"],
             "verdict": "green",
             "note": "HADAG ferry pier 180 m and HVV bus 140 m — both on the block."},
            {"title": "Cycling & car",
             "tiles": ["cycling_network", "parkzone",
                       "car_sharing_reach", "ev_charging_reach"],
             "verdict": "green",
             "note": "Cycleway on the doorstep; inside a Bewohnerparkgebiet (residents get a Bewohnerparkausweis); 4 car-sharing points and 3 EV chargers within 500 m."},
            {"title": "Long-haul travel",
             "tiles": ["airport_reach"],
             "verdict": "amber",
             "note": "HAM is 18 km — a 25-minute S1 ride or a longer taxi."},
            {"title": "Neighbourhood profile",
             "tiles": ["sozialmonitoring_status_commuter",
                       "sozialmonitoring_gesamt_commuter"],
             "verdict": "green",
             "note": "Sozialmonitoring Statusindex is 'hoch' — a stable neighbourhood for a commuter household; no Aufmerksamkeitsgebiet flag."},
        ],
        "highlights_green": [
            {"tile": "commuter_rail_transit",
             "one_line": "S-Bahn 180 m from the door."},
            {"tile": "commuter_ferry_transit",
             "one_line": "HADAG ferry pier 180 m — Elbe crossing without a car."},
            {"tile": "parkzone",
             "one_line": "Inside a Bewohnerparkgebiet — Bewohnerparkausweis covers on-street parking for residents."},
        ],
        "highlights_red": [
            {"tile": "airport_reach",
             "one_line": "HAM is 18 km — plan a 25-minute S1 or taxi for airport trips."},
            {"tile": "regional_rail_reach",
             "one_line": "Nearest RE/RB platform 1.4 km — a short bike ride."},
        ],
    }, ensure_ascii=False)
...
def run(backend, context: dict) -> dict:
    ...
    raise ValueError(
        f"lens_commuter_hamburg_insight: model returned invalid JSON twice "
        f"({type(last_err).__name__}: {last_err})"
    )


if __name__ == "__main__":
    # -- Pre-flight ruling (T18): every Hamburg COMMUTER_LENS tile key must
    # appear in LENS_SECTION_MAP — catches drift between hamburg.py and this
    # template at boot without requiring a live backend.
    from app.cities.hamburg import COMMUTER_LENS as _HH_COMMUTER
    _mapped = []
    for sec in LENS_SECTION_MAP:
        _mapped.extend(sec["tiles"])
    _lens_tile_keys = {t.key for t in _HH_COMMUTER.tiles}
    assert _lens_tile_keys <= set(_mapped), \
        f"Hamburg COMMUTER_LENS has tile keys not in LENS_SECTION_MAP: {_lens_tile_keys - set(_mapped)}"

    # -- LENS_SECTION_MAP covers all 11 Hamburg Commuter tile keys, no
    # orphans, no duplicates. If a new Commuter tile lands in hamburg.py
    # without a section entry, this assert fails at boot.
    _commuter_tile_keys = {
    ...
    _ctx = {
        "address_hint": {"bezirk": "Hamburg-Mitte", "stadtteil": "HafenCity"},
        "tile_contexts": [
            {"key": "commuter_rail_transit", "label": "S+U-Bahn reach",
             "tier": "green", "rule": "S-Bahn ≤500 m OR U-Bahn ≤500 m",
             "numeric": "S-Bahn 180 m"},
            {"key": "airport_reach", "label": "Airport reach (HAM)",
             "tier": "amber", "rule": "≤15 km green",
             "numeric": "18 km"},
        ],
    }
    ...
    # Hamburg-specific safe words in system prompt.
    for _sw in ("HVV", "HADAG", "Sozialmonitoring", "Aufmerksamkeitsgebiet"):
        assert _sw in _sys, f"system prompt missing Hamburg safe-word: {_sw}"
    ...
    assert "Hamburg-Mitte" in _real_user
    assert "HafenCity" in _real_user
    ...
    # Realistic Hamburg Commuter scenario matching the exemplar rollup:
    # amber, green, green, amber, green → (60+100+100+60+100)/5 = 84.
    _commuter_scenario = [{"verdict": "amber"}, {"verdict": "green"},
                           {"verdict": "green"}, {"verdict": "amber"},
                           {"verdict": "green"}]
    assert _compute_fit_score(_commuter_scenario) == 84, \
        _compute_fit_score(_commuter_scenario)
    ...
    # Provide input tile_contexts covering all 11 Hamburg Commuter tiles
    # with predictable per-section rollup outcomes.
    _input_ctx = {
        "address_hint": {"bezirk": "Hamburg-Mitte", "stadtteil": "HafenCity"},
        "tile_contexts": [
            {"key": "commuter_rail_transit",         "tier": "green"},
            {"key": "regional_rail_reach",           "tier": "amber"},
            {"key": "commuter_ferry_transit",        "tier": "green"},
            {"key": "commuter_bus_transit",          "tier": "green"},
            {"key": "cycling_network",               "tier": "green"},
            {"key": "parkzone",                      "tier": "green"},
            {"key": "car_sharing_reach",             "tier": "green"},
            {"key": "ev_charging_reach",             "tier": "green"},
            {"key": "airport_reach",                 "tier": "red"},
            {"key": "sozialmonitoring_status_commuter", "tier": "green"},
            {"key": "sozialmonitoring_gesamt_commuter", "tier": "green"},
        ],
    }
    ...
    # Short-hop transit [ferry=green, bus=green] → green.
    ...
    # Neighbourhood profile [sozialmonitoring_status_commuter=green,
    # sozialmonitoring_gesamt_commuter=green] → green.
    assert _out["lens_insight"]["sections"][4]["verdict"] == "green", \
        "Neighbourhood profile should roll up to green from sozialmonitoring real tiers"
    ...
    print("lens_commuter_hamburg_insight.py selfcheck OK")
