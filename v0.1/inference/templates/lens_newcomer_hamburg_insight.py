"""`lens_newcomer_hamburg_insight` — one-call lens-level executive summary
for the Newcomer lens of the Hamburg address-intelligence tool.
...
Response schema:

    {
      ...
      "sections": [
        {"title": "Daily transit",
         "tiles": ["rail_transit", "ferry_transit", "bus_transit"],
...
# flowing prose. max_tokens is generous to fit 4 sections + up to 6
# highlights each side without truncation.
SAMPLER = {
    "temp":               0.35,
    "top_p":              0.9,
    "repetition_penalty": 1.1,
    # 1400 tokens covers the current 13-tile Hamburg Newcomer payload with 4
...
LENS_SECTION_MAP: list[dict] = [
    ...
    {"title": "Neighbourhood profile",
     "tiles": ["nightlife_density", "sozialmonitoring_status", "sozialmonitoring_gesamt"]},
]
...
# in the tile caveat). `sozialmonitoring_status` and
# `sozialmonitoring_gesamt` DO carry real tiers (Hamburg BSW
# Sozialmonitoring — 4-level Statusindex per Statistisches Gebiet
# ~2200 residents); they only land as unknown when the address falls
# outside a published polygon. The `.get(tier, -1)` default excludes
# any non-ranked value from the rollup so a section of only unknown /
# info tiles rolls up to `unknown`.
...
_SYSTEM = (
    ...
    "of a Hamburg address-intelligence tool. The user is an English-"
    "speaking expat in their first 90 days in Hamburg. You receive a JSON "
    "payload with: (a) the address's Bezirk / Stadtteil hint, (b) a list "
    ...
    "`sozialmonitoring_status` and `sozialmonitoring_gesamt` are real "
    "neighbourhood-status tiers (Hamburg BSW Sozialmonitoring — 4-level "
    "Statusindex per Statistisches Gebiet ~2200 residents; "
    "Aufmerksamkeitsgebiet is a combined Status+Dynamik verdict) and ARE "
    "eligible to be cited as positives or concerns the same way transit "
    "tiles are. "
    ...
    "values (e.g., 'Neighbourhood profile', 'Daily transit', "
    "'Settling-in services', 'Everyday errands') are "
    ...
    "'nightlife density', 'HVV rail access', 'S-Bahn reach'), NOT the "
    ...
    "instead. "
    "No German words except proper names (Bezirk, Stadtteil, HVV, "
    "Bücherhallen, Kundenzentrum, HADAG, Elbe, Sozialmonitoring, "
    "Aufmerksamkeitsgebiet are fine as terminology). "
    ...
    "tiles — amber is the middle ground, not a failure. A Bücherhalle "
    ...
    "driving the verdict. One such tile: `parkzone` (Hamburg resident "
    "permit regime). When it appears in a section's tile list, the note "
    "MUST cite it. Adapt to the tile's rule; example phrasings "
    "(paraphrase to fit the sentence, do not copy verbatim, do not "
    "invent a fee or enforcement hours not present in the payload — "
    "the Hamburg WFS does not publish those): "
...
def build_messages(context: dict) -> list[dict]:
    ...
    Expected `context` shape:
      {
        ...
        "tile_contexts": [
          {"key": "rail_transit", "label": "Rail Transit",
           "tier": "green", "rule": "≥1 S/U-Bahn within 800 m",
    ...
    # stadtteil in the exemplar was being copied into the response by
    ...
    exemplar_assistant = json.dumps({
        "executive_summary": (
            "This Stadtteil reads strong on the Newcomer axes that matter "
            "most in the first 90 days: S-Bahn and ferry piers are on your "
            "doorstep, and international food, coworking and a Bücherhalle "
            "branch are all within 1 km. The main gap is German-class "
            "access — the nearest Sprachschule is 3.7 km away, so factor "
            "a longer commute or a monthly HVV transit pass into your "
            "enrollment plan."
        ),
        "sections": [
            {"title": "Daily transit",
             "tiles": ["rail_transit", "ferry_transit", "bus_transit"],
             "verdict": "amber",
             "note": "S-Bahn and HVV bus are close; HADAG ferry pier is 720 m — walkable but not doorstep."},
            {"title": "Settling-in services",
             "tiles": ["intl_food", "coworking", "english_clinic", "language_school"],
             "verdict": "red",
             "note": "International food and coworking are dense; German-class access is the drag at 3.7 km."},
            {"title": "Everyday errands",
             "tiles": ["library", "packstation", "parkzone"],
             "verdict": "amber",
             "note": "Bücherhalle and Packstation on the block; inside a Bewohnerparkgebiet — residents get a Bewohnerparkausweis."},
            {"title": "Neighbourhood profile",
             "tiles": ["nightlife_density", "sozialmonitoring_status", "sozialmonitoring_gesamt"],
             "verdict": "amber",
             "note": "Sozialmonitoring Statusindex is 'mittel'; fourteen venues within 300 m — dense nightlife may be a positive or negative depending on preferences."},
        ],
        "highlights_green": [
            {"tile": "rail_transit",
        ...
        "highlights_red": [
            {"tile": "language_school",
             "one_line": "Nearest German course is 3.7 km — plan a monthly HVV transit pass."},
            {"tile": "english_clinic",
...
def run(backend, context: dict) -> dict:
    ...
    raise ValueError(
        f"lens_newcomer_hamburg_insight: model returned invalid JSON twice "
        f"({type(last_err).__name__}: {last_err})"
    )


if __name__ == "__main__":
    # -- Pre-flight ruling (T17): every Hamburg NEWCOMER_LENS tile key must
    # appear in LENS_SECTION_MAP — catches drift between hamburg.py and this
    # template at boot without requiring a live backend.
    from app.cities.hamburg import NEWCOMER_LENS as _HH_NEWCOMER
    _mapped = []
    for sec in LENS_SECTION_MAP:
        _mapped.extend(sec["tiles"])
    _lens_tile_keys = {t.key for t in _HH_NEWCOMER.tiles}
    assert _lens_tile_keys <= set(_mapped), \
        f"Hamburg NEWCOMER_LENS has tile keys not in LENS_SECTION_MAP: {_lens_tile_keys - set(_mapped)}"

    # -- LENS_SECTION_MAP covers all 13 Hamburg Newcomer tile keys, no
    # orphans, no duplicates. If a new Newcomer tile lands in hamburg.py
    ...
    _newcomer_tile_keys = {
        ...
        "nightlife_density", "sozialmonitoring_status", "sozialmonitoring_gesamt",
    }
    ...
    _ctx = {
        "address_hint": {"bezirk": "Altona", "stadtteil": "Ottensen"},
        "tile_contexts": [
            {"key": "rail_transit", "label": "Rail Transit",
             "tier": "green", "rule": "≥1 S/U-Bahn within 800 m",
             "numeric": "U-Bahn 240 m · S-Bahn 620 m"},
    ...
    for _sw in ("HVV", "Bücherhallen", "HADAG", "Sozialmonitoring", "Aufmerksamkeitsgebiet"):
    ...
    assert "Altona" in _real_user
    assert "Ottensen" in _real_user
    ...
    # Ottensen sample scenario: amber, red, red, amber, unknown
    # → (60+20+20+60)/4 = 40.
    _ott = [{"verdict": "amber"}, {"verdict": "red"},
            {"verdict": "red"}, {"verdict": "amber"},
            {"verdict": "unknown"}]
    assert _compute_fit_score(_ott) == 40, _compute_fit_score(_ott)
    ...
    # Force a wrong verdict on Daily transit — rollup should reset
    ...
    _input_ctx = {
        ...
        "tile_contexts": [
            ...
            {"key": "parkzone",                 "tier": "amber"},
            {"key": "nightlife_density",        "tier": "info"},
            {"key": "sozialmonitoring_status",  "tier": "green"},
            {"key": "sozialmonitoring_gesamt",  "tier": "green"},
        ],
    }
    _out = run(_b, _input_ctx)
    assert _b.calls == 1, "valid response should not retry"
    assert _out["lens_insight"]["sections"][0]["verdict"] == "green", \
        "rollup should have overridden 'red' back to 'green' for all-green Daily transit section"
    assert _out["lens_insight"]["sections"][1]["verdict"] == "red", \
        "Settling-in services rolls up to red because language_school is red"
    # Neighbourhood profile [nightlife_density=info, sozialmonitoring_status=green,
    # sozialmonitoring_gesamt=green] rolls up to green — sozialmonitoring tiles
    # carry real tiers.
    ...
    print("lens_newcomer_hamburg_insight.py selfcheck OK")
