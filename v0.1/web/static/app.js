...
function render(d){
  ...
  const addr=`<div class="cell edu-cell" data-edu-cat="address"><div class="cell-head"><div class="icon-badge">${ico.home}</div><span class="cell-label">Address</span>${explainBtn('edu-address','Address',addrFields)}</div>
    ...
    <div class="badges">${c.district?`<span class="badge b-dist">${esc(c.district)}</span>`:''}${c.esb?`<span class="badge b-esb">ESB ${esc(c.esb)}</span>`:''}</div>
    <div class="addr-history" data-hist='${esc(JSON.stringify(_histCtx))}'>
      <button type="button" class="pill-btn addr-history-btn">✦ Get History</button>
  ...
  $out.querySelectorAll('.edu-cell').forEach(cell=>cell.addEventListener('click',(e)=>{
    ...
    if(e.target.closest('.addr-history')) return;                   // v0.1: Get History flow
  ...
  $out.querySelectorAll('.addr-history-btn').forEach(btn => btn.addEventListener('click', async (e) => {
    e.stopPropagation();
    const wrap = btn.closest('.addr-history');
    const body = wrap.querySelector('.addr-history-body');
    let ctx;
    try { ctx = JSON.parse(wrap.dataset.hist || '{}'); } catch { ctx = {}; }
    if (!ctx.lat || !ctx.lon) {
    ...
    try {
      ...
      body.innerHTML = `
        ...
        <p class="addr-history-foot">Generated by AI and may display incorrect information.</p>`;
    } catch (err) {
