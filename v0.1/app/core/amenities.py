...
def amenities_near(index: Index, cfg: CityConfig, lon, lat, radius_m=800):
    ...
    # --- Prefer local Geofabrik snapshot (v0.1) ----------------------------
    # If Index loaded a fresh weekly snapshot, serve all 6 amenity buckets
    # from it and skip the heavy combined Overpass query. We still fire a
    # narrow hospital-only Overpass call below for BOD-hospital contact
    # enrichment (the snapshot doesn't include hospitals).
    if getattr(index, "osm_local", None) is not None:
        used_local = True
        oc = index.osm_local
        for cat, _, _ in AMENITIES:
            for f in oc.near(cat, lon, lat, radius_m):
                name = f.get("name") or fallback[cat]
                dkey = (name,) if cat == "transit" else (name, round(f["lat"], 5), round(f["lon"], 5))
                if dkey in seen[cat]:
                    continue
                seen[cat].add(dkey)
                tags = f.get("tags") or {}
                buckets[cat].append({"name": name, "lat": f["lat"], "lon": f["lon"],
                                     "distance_m": f["distance_m"],
                                     "info": _summarize(cat, tags), "source": "osm",
                                     "tags": tags})
        # Hospital contact overlay served from the local snapshot too — the
        # refresh script emits an amenity=hospital / healthcare=hospital
        # bucket, so BOD hospitals still get phone/website/emergency/
        # wheelchair enrichment without any Overpass hop.
        for f in oc.near("hospital", lon, lat, hospital_radius):
            osm_hospitals.append({"lat": f["lat"], "lon": f["lon"],
                                   "tags": f.get("tags") or {}})
    else:
        # --- Overpass round-trip (legacy path) ------------------------------
        # Combined query for all 6 amenity buckets + hospitals in one round trip.
    ...
    for cat, _, _ in AMENITIES:
        ...
        if cat in bod_layers:
            ...
            prov = ("© OpenStreetMap contributors (ODbL) via Geofabrik weekly snapshot"
                    if used_local
                    else "© OpenStreetMap contributors (ODbL)")
            result[cat] = {"count": len(osm_items), "items": osm_items[:25],
                           "provenance": prov}
    if not osm_ok and not used_local:
