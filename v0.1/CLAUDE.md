...
- `/api/lens_insight` — per-lens executive-summary AI panel for all four Life Lenses (Young Family / Newcomer / Quiet Living / Commuter). One Cloudflare call per lens per address; deterministic post-processing (verdict rollup + highlight tone filter + fit_score); cache-versioned so template edits invalidate stale entries. Replaces the removed per-tile `/api/card_insight` dispatcher.

**Inference service is city-agnostic.** `POST /summarize` takes `{template, context, city}`; `city` is accepted but not routed on today. Five templates registered: `history` + one `lens_<slug>_insight` per Life Lens (young_family, newcomer, quiet_living, commuter). All five are remote-eligible by default via `INFERENCE_REMOTE_TEMPLATES` → Cloudflare Workers AI (`@cf/meta/llama-3.1-8b-instruct-fast`). Local backend selected by `INFERENCE_BACKEND=mlx|llama` for dev fallback; prod sets `INFERENCE_LOCAL_BACKEND=off` to skip Qwen entirely (~1 GB RAM reclaimed, no 5-15 s boot delay). `/ready` returns 503 until either the local backend is warm OR remote is present (with local disabled). `_lock` serialises local generation because mlx/llama are not thread-safe. Never bake the model into per-city app images (plan §7.1). Lens templates carry pure selfchecks that assert schema shape, verdict rollup determinism, highlight tone filter, and section-map completeness — do not weaken those asserts.
