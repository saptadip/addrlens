...
# service-symmetric.
#
# app-landing may not exist in the target SHA's compose file (cross-boundary
# rollback across the apex-migration cutover). Query compose config for the
# actual available services and include app-landing only if declared.
SVCS="app app-hh inference"
if "${COMPOSE[@]}" config --services 2>/dev/null | grep -qx app-landing; then
  SVCS="$SVCS app-landing"
fi
"${COMPOSE[@]}" build $SVCS
