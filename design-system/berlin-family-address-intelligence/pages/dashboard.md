...
3. **Result grid** — appears only after a lookup. Flex-row on desktop:
   - **Left stack** (in order): Address · Assigned Grundschule (one per school in catchment) · Kitas metric · Nearest international.
   - **Right column**: Map with catchment polygon overlay, address marker, kita dots, intl-school marker, 800m ring.
4. **Footer** — attribution line (Geoportal Berlin + OSM). Never a card.
...
- **Overlays:** catchment polygon (`#4F46E5`, weight 2.5, fillOpacity .10) · 800m walk ring (`#22C55E`, dashed) · address pin (pink `#EC4899`) · intl-school marker (amber `#F59E0B`) · kita dots (slate `#94A3B8`).
- **After every render:** `requestAnimationFrame(() => mapRef.invalidateSize())` — the flex stretch resolves after layout, Leaflet needs the nudge.
- **Legend:** always visible under the map, on the same `--result-bg` surface.
