...
def _selfcheck():
    ...
    # Overpass mirror rotation — first fails, second succeeds ⇒ return second's result.
    import io
    calls = []
    ...
    def _all_fail(req, timeout=None):
        calls.append(req.full_url); raise TimeoutError("boom")
    urllib.request.urlopen = _all_fail
    try:
        try: overpass("out;", timeout_s=75); assert False, "expected RuntimeError"
