...
Historical prototypes ([`../history/prototypes/phase0`](../history/prototypes/phase0), `phase1`, `phase2`, `phase3`) live under the docs tree; they are frozen reference artefacts. `phase3/` is the direct behavioural ancestor of `v0.1/app/` — many core modules are byte-for-byte ports and their selfchecks preserve that parity.

Governing plan: [`../history/microservice-refactor-plan.md`](../history/microservice-refactor-plan.md) (Ship A, LLM-1, B shipped; Ship C deferred; Ship D in progress). Engineering conventions: §14 of [`../product/product-brief.md`](../product/product-brief.md) — read those before adding features.

## Commands

Environment (root of this dir, `v0.1/`):
...
Two test tiers, both live:

**pytest** — 211 tests under `tests/`, CI-enforced. On a fresh clone, rebuild the venv once:
...
```bash
.venv/bin/pytest                          # unit + contract, no network, ~1 s
.venv/bin/pytest -m integration           # opt-in live Berlin Geoportal (1 test)
.venv/bin/pytest --cov=app                # coverage
```

**Selfchecks** — every module ships a `__main__` pure-assert block; each service has a `selfcheck.py` orchestrator. Complements pytest with sub-second single-module iteration:
